...
.button {
  ...
  cursor: pointer;
    ...
    <body>
        ...
        <form method="post">
            <center>INVIGILATOR ID:<input type="text" placeholder="Enter Invigilator Id" name="id"/>
             <input type="submit" class="button" name='check2' value="CHECK ALLOTMENT"/></center>
        </form>
        <?php
        if(isset($_POST['check2']))
        {
            ...
            echo '<center><b>';
                 $conn= mysqli_connect("localhost","root","","major_project") or die("connection not created");
                            $query="select Room from invigilators where Id='$i'";
                             $result= mysqli_query($conn, $query);
                              if(mysqli_num_rows($result)>0)
                              {
                              if($row= mysqli_fetch_array($result))
                              {  echo "ROOM NO :";
                                   ...
                                   echo $row[0];    
                                       ...
                                       echo "<font color='red'>NOT YET ALLOTED</font>";
                                  }
                             }
                             else
                             {
                                 echo "<font color='red'>INVALID ID</font>";
                             }
